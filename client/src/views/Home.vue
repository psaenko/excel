...
export default {
  ...
  methods: {
    async loadNew() {
      this.isLoading = true
      await axios.get(`http://195.189.227.131:3000/api/requests`).then(({data}) => {
        this.data = data
      })
      setTimeout(() => this.isLoading = false, 2000)
    },
    async changeComment(id) {
      await axios.patch(`http://195.189.227.131:3000/api/comments`, {
        id: id,
    ...
    async changeStatus(id) {
      await axios.patch(`http://195.189.227.131:3000/api/status`, {
        id: id,
        data: event.target.value
      }).then(() => {
